...
let init = async function(networkId) {
    ...
    let nft           = await Nft.at("0xd28067489e84BBE507fFA7E1525B3255D3418aB9");
    ...
    async function signNfts(){
      for(let i = 0; i < requestedTokensAmount; i++){
        ...
        console.log(`token at index ${i} nftAddress: ${requestedTokenMetadata[i][0]} name: ${requestedTokenMetadata[i][1]} quality: ${requestedTokenMetadata[i][2]}`);

        encodedData = encodeNft(requestedTokenMetadata[i][1],
          requestedTokenMetadata[i][2]);
    ...
    function encodeNft(_name, _quality) {
      let name = web3.utils.asciiToHex(_name);
      console.log("name: ",name);
      let bytes32 = web3.eth.abi.encodeParameters(
        ["bytes32", "uint8"], [name, _quality]);
      return bytes32;
    }
