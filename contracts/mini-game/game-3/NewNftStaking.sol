...
contract NewNftStaking is Ownable, IERC721Receiver {
    ...
    function payDebt(uint256 _sessionId, address _address) external onlyOwner {
        ...
        if (_debt > 0) {
            ...
            earning[_sessionId][_address] = earning[_sessionId][_address] + _debt;
		}
	}
    ...
    function deposit(uint256 _sessionId, bytes calldata data, uint8 _v, bytes32 _r, bytes32 _s) external{
        ...
        //verify VRS
        {
            bytes32 _messageNoPrefix = keccak256(abi.encodePacked(_sessionId, _nfts[0], _sp[0], _nfts[1], _sp[1], _nfts[2], _sp[2], nonce, msg.sender));
        ...
        for (uint8 _index = 0; _index < 3; ++_index) {
            ...
            if(_nfts[_index] > 0) {
                ...
                _session.totalSp = _session.totalSp + _sp[_index];
    ...
    function giveBonus(uint256 _sessionId, uint256 _bonusPercent) internal returns(bool) {
        ...
        for(uint _index = 0; _index < 3; ++_index){
	        _interests = _interests + calculateInterest(_sessionId, msg.sender, _index);
        }

	    uint256 _totalBonus = _interests * MULTIPLIER * _bonusPercent / 1000000 / MULTIPLIER;
        ...
        earning[_sessionId][msg.sender] = earning[_sessionId][msg.sender] + _totalBonus;
    ...
    function transfer(uint256 _sessionId, uint256 _index) internal returns(uint256) {
        ...
        if (_interest > 0 && _interest > _crownsBalance) {
            ...
            _session.claimed = _session.claimed + _crownsBalance;
    ...
    function calculateInterest(uint256 _sessionId, address _address, uint256 _index) internal view returns(uint256) {
        ...
        uint256 claimedPerPoint = _session.claimedPerPoint + (_sessionCap - _session.lastInterestUpdate) * _session.interestPerPoint;
	...
	function updateCalculation(uint256 _sessionId) internal returns(bool) {
		...
		_session.claimedPerPoint = _session.claimedPerPoint + (_sessionCap - _session.lastInterestUpdate) * _session.interestPerPoint;
    ...
    function getSignInReward(uint256 _sessionId, uint8 _tagNum, uint8 v, bytes32 r, bytes32 s) external {
        ...
        {
            bytes32 _messageNoPrefix = keccak256(abi.encodePacked(_sessionId, _tagNum, nonce, msg.sender));
            bytes32 _message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", _messageNoPrefix));
