    ...
    ReentrancyGuard
{
    ...
    function newChallenge(uint32 id, bytes calldata data) external onlyZombieFarm {
        ...
        (_earn, _nftAmount, _imgId, _generation, _quality) =abi
            .decode(data, (address, uint8, uint256[5], int8[5], uint8[5]));
    ...
    // Scape NFT Combo doesn't support unstaking.
    function unstake(
        uint256 sessionId,
        uint32 challengeId,
        address staker,
        ...
        nonReentrant
    {
        require(sessionId > 0 && challengeId > 0 && staker != address(0), "empty data");
