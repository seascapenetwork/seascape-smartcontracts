...
contract ScapeNftComboChallenge is ZombieFarmChallengeInterface, Ownable {
    ...
    function newChallenge(uint32 id, bytes calldata data) external override onlyZombieFarm {
        ...
        (_stake, _nftAmount, _imgId, _generation, _quality) =
    ...
    function stake(uint256 sessionId, uint32 challengeId, address staker, bytes calldata data) external override onlyZombieFarm {
        ...
        require(sessionChallenge.levelId > 0, "combo token:no exist session");
        ...
        (nftId, weight) = decodeStakeData(challenge.nftAmount, playerChallenge.nftId, playerChallenge.weight, data);
    ...
    function decodeStakeData(uint8 nftAmount, uint256[5] memory stakedNftId, uint256[5] memory stakedWeight, bytes memory data) internal view returns(uint256[5] memory, uint256[5] memory) {
        if (stakedNftId[0] > 0) {
        ...
        /// Staking amount
        (v, r, s, nftId, weight) = abi.decode(data, (uint8, bytes32, bytes32, uint256[5], uint256[5]));
	...
	function updateBalanceInterestPerToken(uint8 nftAmount, uint256 claimedPerToken, PlayerChallenge storage playerChallenge) internal returns(bool) {
		for (uint8 i = 0; i < nftAmount; i++) {
            playerChallenge.claimedReward = claimedPerToken * playerChallenge.weight[i] / scaler; // 0
        }
    }
    ...
    function calculateInterest(uint256 sessionId, uint32 challengeId, address staker) internal view returns(uint256) {
		...
		if (playerChallenge.weight[i] == 0 || sessionChallenge.amount == 0) {
			return 0;
		}
