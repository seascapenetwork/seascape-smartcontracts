...
contract ScapeNftTokenChallenge is ZombieFarmChallengeInterface, Ownable {
    ...
    event Stake(address indexed staker, uint256 indexed sessionId, uint32 challengeId, uint256 amount, uint256 sessionAmount, uint256 nftId);
    event Unstake(address indexed staker, uint256 indexed sessionId, uint32 challengeId, uint256 amount, uint256 sessionAmount, uint256 nftId);
    ...
    function newChallenge(uint32 id, bytes calldata data) external override onlyZombieFarm {
        ...
        (_stake, _earn, _imgIdAmount, _imgId, _generation, _quality, _burn) = 
            abi.decode(data, (address, address, uint8, uint256[5], int8, uint8, bool));
    ...
    function stake(uint256 sessionId, uint32 challengeId, address staker, bytes calldata data) external override onlyZombieFarm {
		...
		emit Stake(staker, sessionId, challengeId, amount, sessionChallenge.amount, playerChallenge.nftId);
    }

    /// @dev it returns amount for stake and nft id.
    /// If user already staked, then return the previous staked token.
    function decodeStakeData(uint256 stakedNftId, bytes memory data) internal view returns(uint256, uint256) {
        uint256 amount;
